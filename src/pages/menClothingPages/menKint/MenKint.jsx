...
export default function MenKint() {
  ...
  const menKnitCollectionRef = collection(db, "menKint");

  useEffect(() => {
    dispatch(fetchMenData("menKint", menKnitCollectionRef));
  }, [dispatch]);
      ...
      {!loading ? (
        <>
          <div className="w-fit mx-auto flex items-center gap-2 sm:gap-3">
            <span className="text-lg sm:text-xl font-bold">Knit</span>
